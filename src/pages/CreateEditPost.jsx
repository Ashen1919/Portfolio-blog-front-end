    ...
    <div className="flex flex-wrap gap-2">
      {allTags.map((tag) => {
        ...
        return (
          ...
          >
            {selected && <RiCloseLine className="text-xs" />}#{tag.name}
          </button>
        );
      })}
      {allTags.length === 0 && (
...
const CreateEditPost = ({ mode = "create" }) => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-invert max-w-none min-h-[400px] px-4 py-3 focus:outline-none text-gray-300",
      },
    },
  });
    ...
    const loadFormData = async () => {
      try {
        ...
        const [catRes, tagRes] = await Promise.all([
          ...
          axiosInstance.get("/api/tags?page=0&size=100"),
        ]);
